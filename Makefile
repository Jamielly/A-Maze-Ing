...
test-package: package
	...
	cd /tmp && $(CURDIR)/testenv/bin/python -c "from mazegen import MazeGenerator; m=MazeGenerator(20,20,(0,0),(19,19),seed=42,perfect=True); m.generate(); print('WHL OK:', m is not None)"
	rm -rf testenv
	@echo "Testing TAR.GZ..."
	python3 -m venv testenv
	testenv/bin/pip install --index-url $(PIP_INDEX) ./mazegen-*.tar.gz
	cd /tmp && $(CURDIR)/testenv/bin/python -c "from mazegen import MazeGenerator; m=MazeGenerator(20,20,(0,0),(19,19),seed=42,perfect=True); m.generate(); print('TAR.GZ OK:', m is not None)"
	rm -rf testenv

.PHONY: all install run debug lint lint-strict package clean fclean re test-package
